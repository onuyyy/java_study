...
public class MovieSystem {
	...
	static {
		...
		int i = 0;	// 한글자 읽기		 A => 65
		StringBuffer sb = new StringBuffer();		// 데이터가 많은데 한 번에 문자열 결합을 시킬 때 사용한다
		
		while((i=fr.read())!=-1) {	// -1 EDF
...
//		System.out.println(sb.toString());
	...
	public ArrayList movieListData(int page) {
		...
		// size() = 1938		1938/10.0 = 193.8 => 올림 194
		
		int start = (page-1)*10;		// 0, 10, 20, ...
		int end = page*10;				// 0 ~ 10, 10 ~ 20, ...
	...
	public void process() {
		while(true) {
			...
			if(menu==9) {
				...
				System.out.println("상세 볼 영화번호 입력(1~1938) : ");
				int no = scan.nextInt();
