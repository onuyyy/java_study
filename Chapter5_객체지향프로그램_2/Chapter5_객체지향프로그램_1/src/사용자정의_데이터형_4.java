...
public class 사용자정의_데이터형_4 {
	...
	public static void main(String[] args) {
		 ...
		 * 한 메모리에 여러 개 다른 데이터형 모아서 관리
		 * 데이터 관리
		 */
		
		
		Music[] music = new Music[50];		// 사용자정의 데이터형 
		// int[] arr = new int[50] => 사용자정의 데이터형
//		System.out.println(MusicData.title.length);
		for(int i = 0; i < 50; i++) {
			music[i] = new Music();						//	배열을 잡으면 인덱스 번호가 구분자가 된다
		...
		// Music의 목록 출력
		int i =1;
		for(Music m :music) {		// for(int a: arr)
			System.out.println(i+"."+m.title);
			i++;
		}

		Scanner scan = new Scanner(System.in);
		System.out.print("상세 볼 뮤직을 선택");
		int no = scan.nextInt();
		System.out.println("========== 상세 보기 ==========");
		System.out.println("1. 노래명 : "+music[no-1].title);		// 배열이니까 입력 값보다 -1 해야함
		System.out.println("2. 가수명 : "+music[no-1].singer);
		System.out.println("3. 앨범 : "+music[no-1].album);
		
	       String s=music[no-1].state;		// 배열은 0번부터 시작하기 때문에 입력값에서 -1
	        if(s.equals("유지"))
	        	s="-";
	        else if(s.equals("상승"))
	        	s="▲ "+music[no-1].modify;
		...
		// 상승이 가장 많은 노래와 하강이 가장 많은 노래 출력
		System.out.println("========== 상승이 가장 많은 노래 출력 ==========");
		int max = 0;
		int index = 0;
		for(i = 0; i < music.length; i++) {		// 위에서 i 쓰고 있기때문에 초기화만 하고 다시 사용
		...
		System.out.println("1. 노래명 : "+music[index].title);		// 배열이니까 입력 값보다 -1 해야함
		System.out.println("2. 가수명 : "+music[index].singer);
		System.out.println("3. 앨범 : "+music[index].album);
		System.out.println("4. 상태 : "+music[index].state);
		System.out.println("5. 범위 : "+music[index].modify);
		
		
		
		// 하강이 가장 많은 노래와 하강이 가장 많은 노래 출력
